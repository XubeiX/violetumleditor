...
public class CustomPropertyEditor implements ICustomPropertyEditor {
	...
	public CustomPropertyEditor(Object bean) {
		...
		try {
			...
			ResourceBundle rs = ResourceBundle.getBundle(ResourceBundleConstant.NODE_AND_EDGE_STRINGS,
					Locale.getDefault());
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.horstmann.violet.framework.display.clipboard.IPropertyEditor#
	 * getAWTComponent()
	 */
			...
			editor.addPropertyChangeListener(new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent event) {
					try {
						...
						firePropertyStateChanged(
								new PropertyChangeEvent(bean, descriptor.getName(), oldValue, newValue));
	...
	private Component getEditorComponent(final PropertyEditor editor) {
		...
		if (editor.supportsCustomEditor()) {
			 ...
			 * = new JButton(); // if the editor is paintable, have it paint an
			 * icon if (editor.isPaintable()) { button.setIcon(new Icon() {
			 * public int getIconWidth() { return WIDTH - 8; } public int
			 * getIconHeight() { return HEIGHT - 8; }
			 * 
